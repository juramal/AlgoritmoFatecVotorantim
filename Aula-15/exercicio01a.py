...
arquivoEntrada = open('Aula15/empregados.txt')
...
saida = open('Aula15/saida_ex1.txt', 'w')
saida.write(l1)
saida.write(l2)
saida.write(l3)
for i in range(len(usuarios)):
    ...
    saida.write(f'\n{i+1} - {usuarios[i]:15} - {espacoMB:10.2f} - {percentual_uso:10.2%}')

saida.write(f'\nEspaco total ocupado: {espaco_total / mega:5.2f} MB')
saida.write(f'\nEspaco medio ocupado: {(espaco_total / len(usuarios))/ mega:5.2f} MB')
# Fecha o arquivo
saida.close()